  ...
  useEffect(() => {
    ...
    const globalSub = supabase.channel('dashboard-realtime')
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'attendance' }, payload => {
        setAttendance(prev => prev.map(item => item.id === payload.new.id ? { ...item, ...payload.new } : item));
      })
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'students' }, payload => {
        setStudents(prev => prev.map(item => item.id === payload.new.id ? { ...item, ...payload.new } : item));
      })
    ...
    <div className="min-h-screen w-full bg-light pb-24 lg:pb-5 overflow-x-hidden">
      ...
      <div className="mx-auto max-w-[1200px] w-full px-2 sm:px-3 pt-20 sm:pt-24 overflow-x-hidden">
        ...
        <div className="w-full overflow-x-hidden">
          ...
          {isDashboardView && (
            <>
              {tab === 'attendance' && <AttendancePanel students={students} allRecords={processedToday} showAllStatus={showAllStatus} searchQuery={search} isAdmin={isAdmin} adminInfo={adminInfo} today={today} onOpenScanner={() => setScanner(true)} refresh={fetchInitialData} />}
              {tab === 'leaves' && <LeaveRequestPanel students={students} records={attendance} isAdmin={isAdmin} adminInfo={adminInfo} today={today} refresh={fetchInitialData} />}
...
function AttendancePanel({ students, allRecords, showAllStatus, searchQuery, isAdmin, adminInfo, today, onOpenScanner, refresh }: any) { 
  const [name, setName] = useState(''); 
...
function LeaveRequestPanel({ students, records, isAdmin, adminInfo, today, refresh }: any) {
  const [name, setName] = useState('');
...
function ScoresPanel({ students, isAdmin }: any) { 
...
function Analytics({ counts, totalStudents }: any) { 
