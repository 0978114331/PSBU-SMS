...
export function Navbar({ activeTab, isAdmin, userLabel, role, mobileOpen, onTabChange, onScanner, onSignOut, onMobileToggle }: NavbarProps) {
  return <>
    <header className="fixed top-0 z-40 flex w-full items-center justify-between bg-[#2c3e50] px-4 py-2.5 text-white shadow-lg">
      <div className="flex items-center gap-3">
        <div className="flex h-11 w-11 items-center justify-center rounded-full bg-white text-primary"><GraduationCap /></div>
        <b className="text-sm sm:text-base">PSB University</b>
      </div>
      ...
      <div className="flex items-center gap-2 lg:hidden">
        <button className="btn bg-secondary px-3 text-white" onClick={onScanner}><QrCode size={18} /></button>
        <button onClick={onMobileToggle} className="rounded-lg p-2"><Menu size={22} /></button>
      </div>
    </header>
    {mobileOpen && <div className="fixed top-[64px] z-30 w-full bg-[#2c3e50] p-4 shadow-xl lg:hidden">
      ...
      <button className="mt-3 flex w-full items-center justify-center gap-2 p-3 text-red-200" onClick={onSignOut}><LogOut size={17} /> ចាកចេញ</button>
    </div>}
  </>;
}
...
function MobileNavButton({ item, active, onClick, nested = false }: { item: NavItem; active: boolean; onClick: () => void; nested?: boolean }) {
  const Icon = item.icon;
  return <button className={`mb-2 flex w-full items-center gap-2 rounded-lg p-3 font-bold ${nested ? 'justify-start pl-3 text-sm' : 'justify-center'} ${active ? 'bg-primary text-white' : 'bg-white/10 text-white'}`} onClick={onClick}><Icon size={18} />{item.label}</button>;
}
